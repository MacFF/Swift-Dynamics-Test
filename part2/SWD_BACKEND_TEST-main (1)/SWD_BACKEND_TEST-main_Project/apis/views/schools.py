...
class StudentSubjectsScoreAPIView(APIView):
    ...
    def post(request, *args, **kwargs):
        ...
        student_first_name = request.data.get("first_name", None) #first_name ทางซ้ายคือ key ถ้าผู้ใช้ระบุ key ใน json มาเป็น "first_name": "John" ระบบก็จะเอา value ของมันมาถ้าใส่เป็น "fname": "John" จะขึ้น none
        student_last_name = request.data.get("last_name", None)  # function get คือถึงค่า value จาก key ที่ถูกส่งเข้ามา
        subjects_title = request.data.get("subject_title", None) #หากไม่มีข้อมูลนี้จะถูกกำหนดให้เป็น None ซึ่งหมายถึงไม่มีข้อมูล.
        score = request.data.get("score", None)
        
        if (not isinstance(student_first_name, str) or
            not isinstance(student_last_name, str) or
            not isinstance(subjects_title, str) or
            not isinstance(score, (int, float)) or
            score < 0 or score > 100):
            return Response("Invalid payload data", status=status.HTTP_400_BAD_REQUEST)
        
        #- Student's score of each subject must be unique (it's mean 1 student only have 1 row of score of each subject).
        students = Personnel.objects.filter(first_name=student_first_name, last_name=student_last_name)
        subjects = Subjects.objects.filter(title=subjects_title)   #.first() จะคืนค่าแรกที่พบ 
        existing_score = StudentSubjectsScore.objects.filter(student=students[0], subjects=subjects[0]).first()
        # ถ้ามีข้อมูลที่ตรงกับเงื่อนไข ฟังก์ชัน exists() จะคืนค่า True ถ้าไม่มีข้อมูลที่ตรงกับเงื่อนไข ฟังก์ชัน exists() จะคืนค่า False.
        if existing_score:
        ...
        #- If Data Payload not complete return clearly message with bad request status.
        if (
            student_first_name is None or
            student_last_name is None or
            subjects_title is None or
            score is None
        ):
